...
8. **Create**: `git checkout -b {branch}`.
9. **Summary**: branch name, based on which default branch, confirm ready to work.

## See also

For a linked **worktree** instead of a branch checkout, use the `/wt new <branch-name>` slash command (`pi-extensions/git-worktree`). Worktree mode is no longer an opt-in flag on this skill.
