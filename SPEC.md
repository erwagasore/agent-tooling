...
/release                                             → apply a stable release using the computed bump
...
Pre-releases start at `<identifier>.1`. Repeating an identifier on an existing pre-release increments its sequence without bumping the base again (`rc.1` → `rc.2`); changing the identifier keeps the base and resets the sequence to `.1`.
